...
int index(Vec2d &r){
	IVec2d ivr;
	ivr.x=((int)((r.x+Lhalf.x)/dL.x))%Nside.x;
	ivr.y=((int)((r.y+Lhalf.y)/dL.y))%Nside.y;
...
void init(int npart, Vec2d side, double rcut){
	...
	rcut*=(1+1.0e-8); 
	Nside.x = (int)(side.x/rcut);
	Nside.y = (int)(side.y/rcut);
	...
	int Icell, Jcell, NN;
	for (int ix = 0; ix < Nside.x; ++ix) 
	for (int iy = 0; iy < Nside.y; ++iy)   {
