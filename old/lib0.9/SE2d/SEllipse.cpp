...
Vec2d const SEllipse::on_surf_omotopy(Vec2d const v){
	double angle=atan2(v*u[1],v*u[0]);
...
double const SEllipse::dir2angle(Vec2d const& r){
	...
	double theta=atan2(r*u[1],r*u[0]);
...
double const SEllipse::fsurf(Vec2d const& P){
	Vec2d dr=P-r, v( dr*u[0]/a.x , dr*u[1]/a.y ); 
...
{
	Vec2d dr=P-r, v( dr*u[0]/a.x , dr*u[1]/a.y ), sgnv( (v.x<0)?-1:1, (v.y<0)?-1:1 ); 
