...
double Ellipse::perimeter(){return M_PI*(1.5*(a[0]+a[1])-sqrt(a[0]*a[1]));}

double Ellipse::area(){return M_PI*a[0]*a[1];}
...
Vec2d const Ellipse::on_surf_omotopy(Vec2d const &dir)
{
    ...
    return Vec2d( cos(angle)*a[0]*u[0] + sin(angle)*a[1]*u[1] + r);
}
...
Vec2d const Ellipse::Don_surf(double const angle){
    return Vec2d( -sin(angle)*a[0]*u[0] + cos(angle)*a[1]* u[1] );
}

double const Ellipse::dir2angle(Vec2d const& dir)
{
    double angle=atan2(u[1].dot(dir),u[0].dot(dir));
    return atan2( a[0]*sin(angle) , a[1]*cos(angle) );
}
...
{
    Vec2d dr=P-r, v(dr.dot(u[0])/a[0],dr.dot(u[1])/a[1]);
    return v.dot(v)-1;
}
...
double const Ellipse::fsurf(Vec2d const& P, Vec2d const& L)
{
    ...
    Vec2d v(dr.dot(u[0])/a[0],dr.dot(u[1])/a[1]);
    return v.dot(v)-1;
}
...
{
    Vec2d dr=P-r;
    return Vec2d( 2*u[0].dot(dr)*u[0]/a2[0] + 2*u[1].dot(dr)*u[1]/a2[1] );
}

Vec2d const Ellipse::grad(double const angle)
{
    return Vec2d( 2*cos(angle)*u[0]/a[0] + 2*sin(angle)*u[1]/a[1] );
}

Vec2d const Ellipse::Dgrad(double const angle)
{
    return Vec2d( -2*sin(angle)*u[0]/a[0] + 2*cos(angle)*u[1]/a[1] );
}
